...
switch($action){
  ...
  case 'read':
    $page = $_POST['page'];
    $min = $page * 10;
    $consulta = "SELECT * FROM tipsandtricks WHERE vigente = 1 ORDER BY
      CASE WHEN fecha > NOW() THEN 1
           WHEN fecha < NOW() THEN 2
      END ASC, fecha LIMIT $min, 10";
  ...
  break;
  case 'count':
    $consulta = "SELECT COUNT(id_tips) as pages FROM tipsandtricks WHERE vigente=1";
    $result = mysqli_query($conexion, $consulta);
