...
function login_($user, $paswd){
	...
	if ($p->idpersona && $p->verificada){
		...
		$resp[0] = false;
		$resp[1] =  utf8_encode('User o contrase�a incorrecto');
		return $js->encode($resp);	
	}
}
...
function logError($numero, $texto){
 	if (!strcmp('X', $numero)){
		...
		//enviar_error($texto);
		//echo "<strong> NO SE PUDO PROCESAR LA INFORMACI�N, POR FAVOR INTENTELO MAS TARDE SE ESTA CORRIGIENDO EL PROBLEMA </strong>\n";
